...
def test_lockout_at_threshold():
    ...
    assert status.next_allowed_at == datetime(2025, 1, 1, tzinfo=timezone.utc) + timedelta(
        seconds=30
    )
...
def test_lockout_exponential_growth_and_cap():
    ...
    assert (s3.next_allowed_at - datetime(2025, 1, 1, tzinfo=timezone.utc)).total_seconds() == 10
    # fail_count = 4 -> exponent 1 -> 20s
    s4 = compute_lockout(4, cfg=cfg, now=datetime(2025, 1, 1, tzinfo=timezone.utc))
    assert (s4.next_allowed_at - datetime(2025, 1, 1, tzinfo=timezone.utc)).total_seconds() == 20
    # fail_count = 7 -> exponent 4 -> 160s -> capped at 100s
    s7 = compute_lockout(7, cfg=cfg, now=datetime(2025, 1, 1, tzinfo=timezone.utc))
    assert (s7.next_allowed_at - datetime(2025, 1, 1, tzinfo=timezone.utc)).total_seconds() == 100
