...
def test_lockout_at_threshold():
    ...
    status = compute_lockout(5, cfg=cfg, now=datetime(2025, 1, 1, tzinfo=UTC))
    assert status.locked
    assert status.next_allowed_at == datetime(2025, 1, 1, tzinfo=UTC) + timedelta(seconds=30)
...
def test_lockout_exponential_growth_and_cap():
    ...
    s3 = compute_lockout(3, cfg=cfg, now=datetime(2025, 1, 1, tzinfo=UTC))
    assert (s3.next_allowed_at - datetime(2025, 1, 1, tzinfo=UTC)).total_seconds() == 10
    # fail_count = 4 -> exponent 1 -> 20s
    s4 = compute_lockout(4, cfg=cfg, now=datetime(2025, 1, 1, tzinfo=UTC))
    assert (s4.next_allowed_at - datetime(2025, 1, 1, tzinfo=UTC)).total_seconds() == 20
    # fail_count = 7 -> exponent 4 -> 160s -> capped at 100s
    s7 = compute_lockout(7, cfg=cfg, now=datetime(2025, 1, 1, tzinfo=UTC))
    assert (s7.next_allowed_at - datetime(2025, 1, 1, tzinfo=UTC)).total_seconds() == 100
