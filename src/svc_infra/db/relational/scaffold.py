...
def _render_auth_template(name: str, subs: Dict[str, Any]) -> str:
    ...
    txt = pkg.files("svc_infra.db.relational.templates.models_schemas.auth").joinpath(name).read_text(encoding="utf-8")
...
def _render_entity_template(name: str, subs: Dict[str, Any]) -> str:
    ...
    Looks up templates under svc_infra.db.relational.templates.models_schemas.entity/<name> and requires them to exist.
    """
    import importlib.resources as pkg
    txt = pkg.files("svc_infra.db.relational.templates.models_schemas.entity").joinpath(name).read_text(encoding="utf-8")
