...
def _render_auth_template(name: str, subs: Dict[str, Any]) -> str:
    ...
    txt = pkg.files("svc_infra.db.sql.templates.models_schemas.auth").joinpath(name).read_text(encoding="utf-8")
...
def _render_entity_template(name: str, subs: Dict[str, Any]) -> str:
    ...
    """
    import importlib.resources as pkg
    txt = pkg.files("svc_infra.db.sql.templates.models_schemas.entity").joinpath(name).read_text(encoding="utf-8")
